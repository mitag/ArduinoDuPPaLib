...
i2cEncoderLibV2 RGBEncoder(0b1100001);
...
void setup(void)
{
  ...
  RGBEncoder.begin(FLOAT_DATA | WRAP_DISABLE | DIRE_LEFT | IPUP_ENABLE | RMOD_X1 | RGB_ENCODER);
  ...
  RGBEncoder.writeGP1conf(GP_PWM | GP_PULL_EN | GP_INT_DI);
  RGBEncoder.writeGP2conf(GP_PWM | GP_PULL_EN | GP_INT_DI);
  ...
  if (digitalRead(IntPin) == LOW) {
    ...
    if (RGBEncoder.updateStatus()) {
      ...
      if (RGBEncoder.readStatus(RINC)) {
        ...
        lcd.print("   ");
        if ((RGBEncoder.readStatus(RMIN) == 0) && (RGBEncoder.readStatus(RMAX) == 0))
